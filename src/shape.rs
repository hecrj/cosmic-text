...
impl ShapeGlyph {
    fn layout(&self, font_size: f32, x: f32, y: f32, level: unicode_bidi::Level) -> LayoutGlyph {
        let x_offset = font_size * self.x_offset;
        let y_offset = font_size * self.y_offset;
        let x_advance = font_size * self.x_advance;
...
impl ShapeLine {
    ...
    pub fn layout(&self, font_size: f32, line_width: f32, wrap: Wrap) -> Vec<LayoutLine> {
